...
exports.getSummaryReport = async (req, res) => {
    try {
        ...
        const openingBalance = previousTransactions.reduce((sum, t) => {
            return t.transactionType === 'receivable'
                ? sum + cleanAmount(t.totalAmount)
                : sum - cleanAmount(t.totalAmount);
        }, 0);
...
if (format === 'pdf') {
    ...
    const { PDFDocument: PDFLibDocument } = require('pdf-lib'); // Add pdf-lib dependency
    ...
    try {
        ...
        const addHeader = () => {
            ...
            if (customerId && transactions[0]?.customerId?.name) {
                doc.font('Helvetica')
                    .fontSize(12)
                    .fillColor('#374151')
                    .text(`Customer: ${transactions[0].customerId.name}`, leftMargin, 45, { width: usableWidth, align: 'center' });
            } else if (isAdmin) {
                doc.font('Helvetica')
                    .fontSize(12)
                    .fillColor('#374151')
        ...
        const addFooter = (pageNumber) => {
            ...
            doc.moveTo(leftMargin, pageHeight - 50)
        ...
        if (reportData.transactions.length === 0) {
            ...
            reportData.transactions.forEach((t, index) => {
                ...
                if (y + txRowHeight > usableHeight && index < reportData.transactions.length - 1) {
                    ...
                    addTxTableHeader();
                    currentY += txRowHeight;
                    console.log(`New page for transaction ${index + 1}, currentY: ${currentY}, page: ${doc.bufferedPageRange().count}`);
                }
